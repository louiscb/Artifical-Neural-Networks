...
def main():
    ...
    degraded_image = images[9]
    net2 = HopfieldNet(max_iter=400)
    net2.fit(threeimages)
    image_pred = net2.predict(degraded_image, method='sequential', show_energy=True, collect_hundredth_image=True)

    origimage = np.reshape(threeimages[0], (-1, 1024))
    accuracy = calc_element_accuracy(origimage, image_pred)
